async function initFinalePage() {
    ...
    app.innerHTML = `
      <section class="section center-page">
        <div class="card center-card">
          ...
          <h2 style="font-size:20px; margin-top:6px;">Schon abgeschlossen 🎉</h2>
  ...
  app.innerHTML = `
    <section class="section">
      ...
      <div class="card fade-in ws-shell" id="game-mount" style="margin-top:14px;"></div>
    </section>
  `;
  ...
  renderFinaleMission(
    mount,
    { title: day.title, materialPhoto: day.finaleMaterialPhoto, materialQuestion: day.puzzleQuestion },
    {
      onMaterialAnswer: async (answerText) => {
        const { error: answerError } = await supabaseClient.rpc("submit_finale_material_answer", {
          p_token: session.token,
          p_day_id: dayId,
          p_answer_text: answerText,
        });
        if (answerError) showToast(friendlyError(answerError), "error");
      },
      onFinish: async (performanceRatio) => {
...
document.addEventListener("DOMContentLoaded", initFinalePage);
